...
[[pom.xml]]
[source,xml]
.configure the scraml sbt-plugin and the dsl dependency in your maven pom.xml file
----
    <properties>
        <scraml.version>0.4.0</scraml.version>
    </properties>
...
The configuration points the `ramlApi` variable to the relative path to your main RAML file that sits in your `src/main/resources`
