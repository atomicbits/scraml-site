...
We are currently on RAML version 0.8 and we highly focus on json-schema models to represent data transfer objects. We are upgrading to
RAML version 1.0 and its own data representation format as soon as we can.
...
----
addSbtPlugin("io.atomicbits"      % "scraml-sbt-plugin"   % "0.4.6")
----


[[build.sbt]]
[source,scala]
.configure the scraml sbt-plugin in build.sbt
----
...
Finally, <<build.sbt>> in your project. The configuration is usually just a one-liner that points the `scramlRamlApi` variable to
the relative path to your main RAML file that sits in your `src/main/resources` folder. If you configure [scramlfont]#Scraml# in a
submodule, then just configure this variable for that module by adding it in the module's `build.sbt` file. If for whatever reason
the RAML files are not in the `resources` folder, then use the `scramlBaseDir` variable to set another base directory, similar to
how the `scramlRamlApi` variable is set.
...
=== Generic typing in json-schema

Officially, there is no generic typing possible in json-schema because json-schema is not aan object-oriented modelling language. It
...
model JSON data in an object-oriented fashion.
...
[[raml-generic-typing]]
[source,json]
.json-shema extension that enables modelling generic types
----
{
